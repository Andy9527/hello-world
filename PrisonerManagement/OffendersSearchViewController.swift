//
//  OffendersSearchViewController.swift
//  PrisonerManagement
//
...
class OffendersSearchViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //建立GoogleMaps
        let camera = GMSCameraPosition.camera(withLatitude: 34.052235, longitude: -118.243683, zoom: 16.0)
        let myMap = GMSMapView.map(withFrame: CGRect.zero, camera: camera)
        //啟動使用者定位
        ...
        //啟動捲動手勢手指撥動平移相機
        myMap.settings.scrollGestures = true
        //啟動旋轉手勢旋轉相機
        myMap.settings.rotateGestures = true
        //啟動縮放手勢縮放相機
        myMap.settings.zoomGestures = true
        //啟動傾斜手勢來傾斜相機
        myMap.settings.tiltGestures = true
        //設定代理器
        myMap.delegate = self
        //加入地圖
        self.view = myMap

        let point:GMSMarker = GMSMarker()
        let cor = CLLocationCoordinate2DMake(34.052235, -118.243683)
        point.position = cor
        point.title = "Wadell, Calvin"
        point.map = self.myMap
...
extension OffendersSearchViewController:GMSMapViewDelegate
{
